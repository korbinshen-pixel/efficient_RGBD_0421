...
def denormalize_image(tensor):
    """将归一化 tensor 还原为 uint8 numpy BGR 图像（只显示 RGB）"""
    mean = np.array([0.485, 0.456, 0.406])
    std  = np.array([0.229, 0.224, 0.225])
    img = tensor.cpu().permute(1, 2, 0).numpy()  # [H, W, C]
    img = img[:, :, :3]  # 只取前 3 通道（RGB）
    img = img * std + mean
...
def project_3d_axes(img, R, t, K, axis_len=0.05, y_shift_rel=0.0):
    ...
    pts_cam = (R @ origin.T + t.reshape(3, 1)).T
    proj = (K @ pts_cam.T).T
...
def draw_3d_bbox_corners(img, R, t, K, model_points,
                         ...
                         y_shift_rel=0.0):
    """
    投影 3D bbox 角点，仅用于可视化。
    """
    h, w = img.shape[:2]
    pts_3d = model_points.astype(np.float32)

    center = pts_3d.mean(axis=0, keepdims=True)
    pts_local = pts_3d - center
    scales = np.array([scale_x, scale_y, scale_z], dtype=np.float32)
    pts_scaled = pts_local * scales + center

    pts_cam = (R @ pts_scaled.T + t.reshape(3, 1)).T
    proj = (K @ pts_cam.T).T
...
def plot_rot_hist(rot_list, save_path, clip_deg=10.0):
    ...
    fig, ax = plt.subplots(figsize=(9, 5))
    if len(arr_clip) > 0:
        bins = np.linspace(0, clip_deg, 41)
        counts, edges, patches = ax.hist(
            arr_clip, bins=bins,
...
class PoseEvaluator:
    def __init__(self, model_points, object_diameter):
        ...
        self.add_threshold  = 0.1 * object_diameter
    ...
    def compute_translation_error(self, pred_t, gt_t):
        return np.linalg.norm(pred_t - gt_t) * 100
...
def evaluate_model(model_path, config, use_add_s=False,
    ...
    with torch.no_grad():
        ...
        if len(test_dataset) > 0:
            print("\n🔍 诊断第一个样本的 anchor 分布...")
            s = test_dataset[0]
            image0 = s['image'].unsqueeze(0).to(device)   # [1,4,H,W]
            scores0 = model(image0)['class'][0, :, 0].cpu().numpy()
            print(f"  置信度 > 0.5: {np.sum(scores0 > 0.5)} / {len(scores0)}")
            print(f"  最高置信度: {scores0.max():.4f}  Top-5: {np.sort(scores0)[-5:][::-1]}\n")

        for idx in tqdm(range(len(test_dataset)), desc="评估进度"):
            sample         = test_dataset[idx]
            image_tensor   = sample['image']          # [4,H,W]
            ...
            image_in = image_tensor.unsqueeze(0).to(device)  # [1,4,H,W]

            preds      = model(image_in)
            ...
            pred_rotation = np.eye(3, dtype=np.float32)
            pred_t        = np.zeros(3, dtype=np.float32)
            ...
            if detected:
                ...
                try:
                    rot6d = preds['rotation'][0, best_idx].cpu().numpy()
...
if __name__ == '__main__':
    ...
    model_files = sorted(checkpoints_dir.glob('best_model_phi*_rgbd.pth'))
